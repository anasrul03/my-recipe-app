...
struct ListByCategories: View {
    ...
    var body: some View {
        ...
        }.navigationTitle(selectedCategory).toolbar(showNavigationBar ? .visible : .hidden, for: .tabBar).onDisappear(perform: {
    ...
    func performAPICall() async throws -> [MealModel]? {
        guard let url = URL(string: "https://www.themealdb.com/api/json/v1/1/filter.php?c=\(selectedCategory)") else{
...
#Preview {
    ListByCategories(selectedCategory: "Beef")
}
